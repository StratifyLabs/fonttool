...
int BmpFontManager::convert_directory(const String & dir_path, bool overwrite){
	Dir dir;
	String path;
	String font_bmp;
	String font_sbf;
...
int BmpFontManager::generate_map(const String & path){
	...
	m_generator.generate_font_file(String());
	return 0;
}

int BmpFontManager::convert_font(const String & path){
...
int BmpFontManager::populate_lists_from_font(const var::String & font_path){
	...
	for(u32 i=0; i < font.character_count(); i++){
		m_generator.character_list().push_back( font.character(i) );
		m_generator.bitmap_list().push_back( font.character_bitmap(i) );
	}
...
int BmpFontManager::populate_kerning_pair_list_from_bitmap_definition(const File & def){
	...
	while( (line = def.gets()).is_empty() == false ){
		...
		if( tokens.at(0) == "kerning" ){
			Tokenizer first(tokens.at(1),Tokenizer::Delimeters("="));
			Tokenizer second(tokens.at(2),Tokenizer::Delimeters("="));
			Tokenizer amount(tokens.at(3),Tokenizer::Delimeters("="));
...
Bitmap BmpFontManager::get_bitmap(const Bmp & bmp, bmpfont_char_t c){
	...
	for(j=0; j < height; j++){
		...
		for(i=0; i < width; i++){
			...
			result.set_pen(Pen().set_color(bitmap_color));
...
int BmpFontManager::load_bmp_characters(const File & def){
	...
	while( !(str = def.gets()).is_empty() ){
		Tokenizer t(str, Tokenizer::Delimeters(" \n"));
		if( t.size() > 0 ){
			strm = t.at(0);
			memset(&d, 0, sizeof(d));
			if( strm == "char" ){
...
int BmpFontManager::load_char(bmpfont_char_t & c, const Tokenizer & t){
	...
	for(i=1; i < t.size(); i++){
		Tokenizer m(t.at(i), Tokenizer::Delimeters("="));
		if( m.size() == 2 ){
			str = m.at(0);
			val = m.at(1).to_integer();
			if( str == "id" ){ c.id = val; }
