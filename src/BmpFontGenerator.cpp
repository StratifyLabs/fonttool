...
int BmpFontGenerator::generate_map_file(const sg_font_header_t & header, const var::Vector<Bitmap> & master_canvas_list){
	...
	if( map_output.create(m_map_output_file, File::IsOverwrite(true)) < 0 ){
...
String BmpFontGenerator::parse_map_line(const var::String & title, const String & line){
	Tokenizer tokens(line, Tokenizer::Delimeters(":\n)"));
...
int BmpFontGenerator::import_map(const String & map){
	...
	header.version = get_map_value(header_lines, "version").to_long(String::base_16);
	if( header.version == 0 ){
	...
	for(u32 i=0; i < kerning_lines.count(); i++){
		Tokenizer tokens(kerning_lines.at(i), Tokenizer::Delimeters(" "));
		if( tokens.count() != 4 ){
		...
		if( character_lines.count() > 0 ){
			...
			for(sg_size_t h = 0; h < character.height; h++){
				...
				for(sg_size_t w = 0; w < character.width; w++){
					...
					canvas.set_pen(Pen().set_color(color));
