...
public:
	...
	int convert_font(const String & path);
	int convert_directory(const String & dir_path, bool overwrite = false);
	int generate_map(const String & path);
...
private:
	...
	int populate_lists_from_font(const String & font_path);
